...
$query_up_email=sprintf("UPDATE t_user_info set active=1,points=200,provenienza='ref6', reg_date='%s' where t_user_info.user_id='%s' and active=0; ",
$conn->real_escape_string($today),
$conn->real_escape_string($uidPal));
$results=$conn->query($query_up_email);

echo  "<div class='contOk'>  <h2 class='perfect'>Grazie, il tuo account è stato attivato hai ricevuto il bonus di 200 bytes!</p></h2></div>";
